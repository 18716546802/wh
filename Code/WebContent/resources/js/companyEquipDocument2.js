...
function addDocument(){
	...
	showPage("/WH/jsp/addEquipDocument.jsp");
...
function editDocument(){
	...
	showPage("/WH/jsp/editEquipDocument.jsp");
				...
				success:function(data){	
					if(data.success){
						...
						showPage("/WH/jsp/companyEquipDocument2.jsp");
			   	    	//window.location.href="/WH/jsp/companyEquipDocument2.jsp";				 
					}else{
...
$(document).ready(function(){
	...
	var f0301=JSON.parse(localStorage.getItem("f0301"));
...
function showJqgrid(grid_data){	  
	...
	jQuery(grid_selector).jqGrid({
		...
		datatype: "local",																														    
		height: 500,
		colNames:['ID','ID','安全附件名称','安全附件检测时间', '检测结论', '检测报告编号','附件检测周期','下次检测时间','备是否过期'],
		colModel:[			
